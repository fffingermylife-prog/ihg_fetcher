...
async def fetch_usd_rate(page, currency):
    ...
    try:
        ...
        if result and result.get("ok") and result.get("data"):
            ...
            # IHG 接口对 CNY/EUR 等"品牌定制汇率"币种会同时返回两条:
            #   - source=K: 品牌专用, 实测 CNY 的 K 源停留在 2022-11 的过期值 11.4745,
            #               导致大陆酒店 cash_price_usd 虚高 ~78 倍, 触发 💎 高 CPP 误推
            #   - source=P: 官方主源, 才是当前实时汇率 (CNY 实测 0.14649)
            # 必须优先取 P, 取不到再 fallback 第 0 条
...
async def _fetch_paired(page, hotel_code, windows):
    """2 路同 page 配对并发 (Step 3 行为, SAME_PAGE_INFLIGHT=2 时使用)
...
async def _fetch_pipelined(page, hotel_code, windows, max_inflight):
    ...
    Bug 1+2 修复 (PR #8) 兼容: task 级重试机制
    - fetch_calendar 失败抛 FetchError, 单 task 重试 1 次 (与 paired 模式语义对齐)
    - 重试 sleep 在 sem 外: 避免占着 slot 不放阻塞其他 task
    - asyncio.gather 默认: 任一 task 最终失败 → 取消其他 pending → worker 标记酒店失败重试
            ⇒ 不会出现"部分 task 成功部分失败 + 当成空数据 → 误判售罄"的脏数据
    """
    TASK_RETRIES = 1  # 单 task 失败后重试次数 (1 = 总共试 2 次, 与 paired 模式 WINDOW_RETRIES 对齐)
    ...
    async def _do(ws, we, points_mode):
        ...
        for attempt in range(TASK_RETRIES + 1):
            try:
                ...
                if attempt < TASK_RETRIES:
                    # 重试 sleep 必须在 sem 外, 否则占着 slot 阻塞其他 task
        ...
        # 重试用完仍失败 → 抛, gather 会取消其他 pending tasks 并向上抛, 让 worker 标记酒店失败
        mode_str = "points" if points_mode else "cash"
        raise FetchError(
            f"task {ws}~{we} {mode_str} failed after {TASK_RETRIES + 1} attempts: {last_err}"
        )
...
async def main():
    ...
    #
    # 三道安全锁 (避免主循环死循环, 来自实战经验 — 500+ 酒店跑完后偶尔重新循环):
    ...
    async with async_playwright() as p:
        ...
        while remaining:
            ...
            if batch_requeue:
                # 双重过滤: 不在本批结果里 + 不在已完结集合里 (防 batch_requeue 含本批 success 酒店)
